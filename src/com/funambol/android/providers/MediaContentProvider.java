...
public class MediaContentProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        return context.getContentResolver().delete(mediaUri, selection, selectionArgs);
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        String allFields[] = {_ID, _DATA, MIME_TYPE, getBucketColumn(),
            DATE_ADDED, DATE_MODIFIED, SIZE, DISPLAY_NAME};
        ...
        if (buckets != null) {
            ...
            for(int i=0;i<buckets.length;++i) {
                ...
                whereClause.append(getBucketColumn()).append("='")
        ...
        if (supportedMimeTypes != null) {
            ...
            for(int i=0; i<supportedMimeTypes.length; i++) {
                ...
                whereClause.append(MediaColumns.MIME_TYPE).append("='")
                        .append(supportedMimeTypes[i]).append("'");
            }
            whereClause.append(") ");
        }
        ...
        if(projection == null) {
            ...
            // Regardless of the fields specified by the caller, we need some to
            // adjust the result
            projection = addFieldToProjection(projection, MediaColumns.SIZE);
            projection = addFieldToProjection(projection, MediaColumns.DISPLAY_NAME);
            projection = addFieldToProjection(projection, bucketColumnName);
            projection = addFieldToProjection(projection, _DATA);
        }
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            try {
                ...
                do {
                    ...
                    try {
                        // First of all check if the file is actually on disk
                        String fullName = cursor.getString(cursor.getColumnIndexOrThrow(_DATA));
                        if (Log.isLoggable(Log.TRACE)) {
                            Log.trace(TAG_LOG, "Checking if file " + fullName + " exists");
                        }
                        fa = new FileAdapter(fullName);
                        fileExists = fa.exists();
                    } catch (IOException ioe) {
                        Log.error(TAG_LOG, "Cannot access file on disk", ioe);
                    }
                    ...
                    if (fileExists) {
                        for(int c=0;c<cursor.getColumnCount();++c) {
                            ...
                            if (SIZE.equals(columnName)) {
                                try {
                                    size = computeSize(cursor);
                                    value = "" + size;
                                    //filter by file size
                                } catch (Exception e) {
                                    Log.error(TAG_LOG, "Cannot get item size from file, using db size", e);
                                }
                            } else if (DISPLAY_NAME.equals(columnName)) {
                                itemName = value;
                            }
                            row[c] = value;
                        }
                        try {
                            ...
                            Log.error(TAG_LOG, "Cannot close file adapter", e);
                        }
                        res.addRow(row);
                    }
    ...
    private String[] addFieldToProjection(String projection[], String columnName) {
        ...
        // If we get here, projection does not include the column, add it
        String res[] = new String[projection.length + 1];
        for(int i=0;i<projection.length;++i) {
            res[i] = projection[i];
        }
        res[res.length - 1] = columnName;
        return res;
    }
}
