...
public class CalendarManagerTest extends AndroidBaseTest {
    ...
    private CalendarAppSyncSource appSyncSource;
    ...
    public void setUp() throws Exception {
        ...
        if(AndroidController.getNativeAccount() == null) {
            appInitializer.init(UnitTestActivity.getInstance());
            try {
                Thread.sleep(5000);
            } catch (Exception e) {}
        }
        ...
        appSyncSource = (CalendarAppSyncSource)appSyncSourceManager
                .getSource(AppSyncSourceManager.EVENTS_ID);
        ...
        if (appSyncSource == null) {
            appSyncSource = new CalendarAppSyncSource(getContext(), "Calendar");
        } else {
            appSyncSourceConfig = (CalendarAppSyncSourceConfig)appSyncSource.getConfig();
        }

        // Make sure the funambol calendar exist
        long id = appSyncSource.createCalendar();
        appSyncSourceConfig.setCalendarId(id);
